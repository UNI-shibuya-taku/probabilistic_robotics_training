...
class KldMcl(Mcl):
    def __init__(self, envmap, init_pose, max_num, motion_noise_stds = {"nn":0.19, "no":0.001, "on":0.13, "oo":0.2},
        ...
        self.widths = widths   # xyθの幅
        self.max_num = max_num # 粒子の上限
    ...
    def draw(self, ax, elems):
        super().draw(ax, elems)
        elems.append(ax.text(-4.5, -4.5, "particle:{}, bins{}".format(len(self.particles), self.binnum), fontsize = 10))
...
tuple( math.floor(e) for e in np.array([-0.1, 2.1, 3.0]).T/np.array([0.2, 0.2, math.pi/18]).T )
